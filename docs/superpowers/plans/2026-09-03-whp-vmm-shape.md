...
- [ ] **Step 8: P8 — partition time suspend freezes the TSC** (`q17`): create, run a guest that halts (`None` mode so it exits), `read_reg(Tsc)` → `suspend_time()` → host sleep 50 ms → `read_reg(Tsc)` again: equal; then run once more and read: advanced. (Task 0.2 already observed the partition *reference* clock behaving this way — 845 → 845 across 20 ms, and reading 0 until a VP has run. Record that beside P8 rather than re-deriving it.)

- [ ] **Step 8b: P9 — which of the page's two 256-bit bitmaps is in-service and which is trigger-mode** (`q18`)

**This one blocks Stage 3 and nothing earlier, but it must be measured here, in the task whose job is measuring.** Task 0.2 pinned the APIC state page as the packed `HV_X64_INTERRUPT_CONTROLLER_STATE` and confirmed by measurement that a requested vector lands in the request bitmap at the predicted word and bit. That refutes most orderings — but **an in-service ↔ trigger-mode swap survives every assertion made so far, survives a round trip in both directions, and is invisible to any symmetric test**, because a conversion that reads and writes through the same wrong mapping agrees with itself. Stage 3 converts this page to and from `cpu/apic.rs` in both directions during a live transfer, so a swap there would corrupt guest interrupt state with nothing to catch it.

Pin it with an **asymmetric** observation — one where the platform itself treats the two fields differently, so agreeing-with-yourself is not enough:
- Deliver a **level-triggered** vector via `request_interrupt` with `trigger_mode` level, read the page, and assert the bit appears in the trigger-mode bitmap and NOT in the field the layout calls in-service; then deliver the same vector edge-triggered and assert it does not.
- Or use priority arbitration, which reads in-service and ignores trigger-mode: raise a vector, let the guest accept it without signalling end-of-interrupt, and assert the in-service bitmap carries it while trigger-mode does not.
Either settles it. Record which you used and the raw words.

Also still unpinned and worth recording as such rather than assuming: which of words 29, 30 and 31 is the error-status register versus the two halves of the interrupt-command register, and words 38–43 entirely. Note in the document that `ApicRegister` currently declares BOTH an `Esr` at word 29 and an `ErrorStatus` at word 39 — two error-status fields, only one of which can be right, which is a transcription smell to resolve when the tail is pinned.

- [ ] **Step 8c: pin the software-disabled APIC as a fact, not a workaround**

Task 0.2 found that a reset APIC is software-disabled and **drops** a fixed vector on arrival, with `WHvRequestInterrupt` still returning success — its test had to enable the spurious-interrupt register first to see anything land. The existing probe document (`docs/whp-platform-probe-2026-08-27.md` §10) says only that a fixed vector is "gated by" the software-enable, which is equally consistent with the platform latching it and delivering later. Those are different platforms to build on. Assert the stronger reading directly: request a vector with the APIC disabled, assert the call returns success AND the request bitmap stays empty, then enable and assert it lands. Record the consequence in the document: **a page conversion must carry the spurious register's enable bit, or restoring a page silently disables the guest's APIC.**
