...
- [ ] **Step 4: Baseline Alpine on the WHP engine at head, three runs**

Run (each ≤ 300 s wall; one after another, never concurrently — one partition per process and one CPU-hungry alarm thread each; no `cargo build` running anywhere on the host meanwhile):
```powershell
$env:ALPINE_ISO="<the original ISO>"; $env:ALPINE_PROBE_PATIENCE_SECS="300"
cargo run --release -q -p rusty_box_whp_engine --example alpine_probe *> baseline-alpine-whp-1.log
```
Record from each log: every `milestone` line's wall, the `RESULT exits=` line (exits by class), and from `RESULT platform …` both `runtime_total_ms` and `hypervisor_ms`. The head engine's in-run share is `(runtime_total_ms − hypervisor_ms) / runtime_total_ms` — `HypervisorRuntime100ns` is the hypervisor's OVERHEAD share of the total (`rusty_box_whp/src/partition.rs:395-400`), so guest time is the difference; and neither is wall time, which is why Stage 1 adds a literal in-run clock (Task 1.5).

- [ ] **Step 5: Baseline the interpreter on Alpine, three runs, same harness**

Run: `$env:ALPINE_ENGINE="interpreter"; cargo run --release -q -p rusty_box_whp_engine --example alpine_probe *> baseline-alpine-interp-1.log` ×3. Record wall to `login:`; the median is what Task 1.9 compares against (the last measurement, 2026-08-29, was 69–71 s through a different harness; it is superseded by this row).

- [ ] **Step 6: Baseline DLX, three runs**

Run: `$env:DLX_WHP_PATIENCE_SECS="60"; cargo run --release -q -p rusty_box_whp_engine --example dlx_whp` ×3. Record wall to `login:` (expect ~2.0 s).
...
In `rusty_box_whp/src/lib.rs`, above the existing Send block: `#![forbid(unsafe_code)]` at the crate root. This is the test — the crate either has no `unsafe` left or it does not compile.

- [ ] **Step 2: Run to verify it fails** — `cargo check --release -p rusty_box_whp`. Expected: errors at every `unsafe` still in the crate (today: `sys/windows.rs`'s blocks and the `unsafe fn map_borrowed` signature in partition.rs).

- [ ] **Step 3: Perform the split**

`git mv` the five files (history matters; do not retype them). The new crate's `lib.rs` is today's `sys.rs` with `mod imp;` becoming the two `#[cfg]`-selected modules and `pub mod error; pub mod vcpu;` added. Its `Cargo.toml` mirrors `rusty_box_whp`'s: same `[target.'cfg(windows)'.dependencies.windows-sys]` block (version 0.61, feature `Win32_System_Hypervisor`), `rusty_box_core` with `std`, `tracing`, workspace inheritance for version/edition/license/authors/repository/homepage, and NO `[lints] workspace = true` (the workspace table denies `unsafe_code`, which this crate exists to allow; instead it carries its own crate-level `#![expect(unsafe_code, reason = "…")]` exactly as `rusty_box_whp/src/sys/windows.rs` does today). `map_borrowed`, the one `unsafe fn` signature in partition.rs, moves to the sys crate as well — the sys crate is where a caller-supplied-pointer contract belongs; `Partition::map_borrowed` becomes a safe wrapper whose own doc names the invariant its caller already discharges.
...
In `xtask/src/ci.rs`, change `("rusty_box_whp/src", 38)` to `("rusty_box_whp/src", 0)` and add `("rusty_box_whp_sys/src", <the exact count the ratchet reports>)`. Run `cargo xtask ci` and read the doctrine-ratchets step's own numbers rather than guessing the count.

**And add a test step for the new crate.** `vcpu.rs` and `error.rs` carry unit tests today, and the "WHP leaf tests" step (`cargo test --release -p rusty_box_whp`, ci.rs:521) will no longer reach them once those files move — a test that stops running is the "compiles clean ≠ reached" trap. Add a `Step { name: "WHP sys tests", args: &["test", "--release", "-p", "rusty_box_whp_sys"], … }` immediately before it, and confirm the moved tests appear in that step's output by name.
...
git commit -m "refactor(whp): the platform seam is its own crate, and the wrapper forbids unsafe"
```
...
- [ ] Alpine on fast mode via `alpine_probe`, three runs, 300 s patience, the baseline ISO: reaches `login:`; wall < the interpreter's MEDIAN from the baseline document (three `alpine_probe` runs, Task 0.1 Step 5 — like for like); `RESULT in_run_share phase="Linux version".."login:"` ≥ 0.90 in each run (G1, measured as `Instant` deltas around `vcpu.run()`); `guest_share` over the same window reported beside it as the platform's cross-check.
- [ ] The same three Alpine runs: `RESULT fatal_signatures=0` (no `Oops`, no `general protection`, no `unexpected_intr` in any line seen).
- [ ] `cargo xtask ci` green; the `unsafe` token baselines exactly as Task 0.2 left them (`rusty_box_whp/src` 0 under `forbid`, `rusty_box_whp_sys/src` at its bound-entry-point count, `rusty_box_whp_engine/src` 1); `Send` assertions present for `Vcpu`, `VpCounters`, `Emulator<(), WhpEngine>`, `VcpuControl`, `VcpuThread<()>`, `DeviceThreadControl`, `FastMachine<()>`.
