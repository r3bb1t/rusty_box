...
No supertraits. No `Any`, no `Send`, no `'static`. Just the callbacks with default no-ops, plus a bitmask method for runtime enable/disable:

```rust
pub trait Instrumentation {
    /// Which callback categories this tracer is interested in.
    /// The registry checks this to skip dispatch entirely for disabled categories.
    /// Default: all enabled. Override to disable categories or toggle at runtime.
...
// Unit type: nothing active, nothing dispatched. Compiles to zero.
impl Instrumentation for () {
    fn active_hooks(&self) -> HookMask { HookMask::empty() }
}
```

For tuples, `active_hooks` ORs the masks of all elements:

```rust
// Generated by macro for (A, B):
fn active_hooks(&self) -> HookMask {
    self.0.active_hooks() | self.1.active_hooks()
}
```

The registry caches the combined mask. Callers recompute it via `refresh_hook_mask()` after mutating tracer state that affects `active_hooks()` output.
