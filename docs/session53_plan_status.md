...
1. **VPCONFLICTD off-by-one** (): Loop `i < index-1` should be `i < index`
2. **KSHIFTLW threshold** (): `count < 15` should be `count < 16`
3. **VPSRLQ shift-by-64 UB** (): `shift_64 > 64` allows UB at shift==64
4. **VPSRAVQ same UB** (): `shift > 64` instead of `> 63`
