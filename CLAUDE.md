...
- ✅ **Session 53: CRITICAL timer fix + ATAPI performance + IDENTIFY + CMOS clock**:
  - **CRITICAL FIX: Timer fire check `==` → `>=`** (`pc_system.rs:254`): `countdown_event()` used `ticks_total == time_to_fire` to detect expired timers. Bochs uses `<=` (timeToFire <= ticksTotal). If ticks_total overshot time_to_fire (countdown period > timer period), the `==` check never matched and the timer was **permanently missed**. This was THE root cause of Alpine stalling after kernel init — LAPIC timer interrupts never fired during HLT, so the scheduler never ran. For continuous timers, `time_to_fire` now advances past `ticks_total` in a while loop to handle multi-period overshoot. Result: 83K+ LAPIC timer fires, kernel progresses through full init into userspace.
  - **ATAPI REP INSW bulk I/O** (`io.rs`, `harddrv.rs`, `mod.rs`): Added `bulk_read_data()` to `BxHardDriveC` and `inp_bulk()` to `BxDevicesC`. REP INSW for IDE data ports (0x1F0/0x170) now reads up to 2048 bytes per call instead of 2 bytes per I/O dispatch. Handles ATAPI lazy-load, sector transitions, and DRQ completion in-loop. ~100-1000x speedup on large ATAPI transfers (X.509 certificate loading visibly faster).
  - **ATAPI IDENTIFY PACKET DEVICE fix** (`harddrv.rs`): Added missing words 48 (dword I/O), 66-68 (PIO/DMA cycle times), 71-72 (packet/service times). Word 65 corrected from 0x02E8 to 0x00B4 matching Bochs.
  - **CMOS real-time clock** (`cmos.rs`): Uses `SystemTime::now()` instead of hardcoded 2025-01-01. Fixes "Clock skew detected" warnings. Century/weekday computed by `update_clock()`.
  - **Instruction batch size** (`emulator.rs`): Increased from 10K to 100K. ~10x less batch overhead.
  - **Multi-sector CD-ROM read** (`harddrv.rs`): Added `read_cdrom_blocks()` for reading multiple consecutive sectors in one file I/O call.
  - **DLX regression**: PASS.
  - **Remaining**: BAD signature still present when BMI1 enabled (AVX2 SHA-1 path) — BMI1 disabled as workaround. `stat: can't stat '68608'` cosmetic errors from mdev parsing CD-ROM size.
