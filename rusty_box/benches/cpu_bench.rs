...
const FINAL_ENTRY_32BIT: &[&[u8]] = &[
    &[0x66, 0x01, 0xD8],
    &[0xF2, 0x0F, 0xE6, 0xC1],
    &[0x66, 0x0F, 0x38, 0x00, 0xC1],
    &[0x66, 0x0F, 0x3A, 0x0F, 0xC1, 0x05],
];

const FINAL_ENTRY_64BIT: &[&[u8]] = FINAL_ENTRY_32BIT;

const LOCK_PREFIX_ALLOWED_32BIT: &[&[u8]] = &[&[0xF0, 0x01, 0x18], &[0xF0, 0x31, 0x18]];

const LOCK_PREFIX_ALLOWED_64BIT: &[&[u8]] = LOCK_PREFIX_ALLOWED_32BIT;
...
fn bench_decode_final_entry_tables(c: &mut Criterion) {
    c.bench_function("decode_final_entry_tables_32", |b| {
        b.iter(|| {
            for instr_bytes in FINAL_ENTRY_32BIT {
                let _ = black_box(fetch_decode32(black_box(instr_bytes), true));
            }
        })
    });

    c.bench_function("decode_final_entry_tables_64", |b| {
        b.iter(|| {
            for instr_bytes in FINAL_ENTRY_64BIT {
                let _ = black_box(fetch_decode64(black_box(instr_bytes)));
            }
        })
    });
...
fn bench_decode_lock_prefix_allowed(c: &mut Criterion) {
    c.bench_function("decode_lock_prefix_allowed_32", |b| {
        b.iter(|| {
            for instr_bytes in LOCK_PREFIX_ALLOWED_32BIT {
                let _ = black_box(fetch_decode32(black_box(instr_bytes), true));
            }
        })
    });

    c.bench_function("decode_lock_prefix_allowed_64", |b| {
        b.iter(|| {
            for instr_bytes in LOCK_PREFIX_ALLOWED_64BIT {
                let _ = black_box(fetch_decode64(black_box(instr_bytes)));
            }
        })
    });
...
criterion_group!(
    ...
    bench_decode_32bit,
    bench_decode_64bit,
    bench_decode_single_nop,
    bench_decode_sse,
    bench_decode_final_entry_tables,
    bench_decode_lock_prefix_allowed
);
