...
pub(in crate::cpu) fn f64_range(
    ...
    let mut sign_a = sign_f64(a);
    let exp_a = exp_f64(a);
    let sig_a = frac_f64(a);
    let sign_b = sign_f64(b);
    let exp_b = exp_f64(b);
    let sig_b = frac_f64(b);

    if f64_is_signaling_nan(a) {
        return softfloat_propagate_nan_f64(a, 0, status);
    }
    if f64_is_signaling_nan(b) {
        return softfloat_propagate_nan_f64(b, 0, status);
    }

    let a_is_nan = is_nan_f64(a);
    let b_is_nan = is_nan_f64(b);
    ...
    let mut z = if b_is_nan {
        ...
        let (tmp_a, tmp_b) = if is_abs {
            sign_a = false;
            (a & !0x8000_0000_0000_0000, b & !0x8000_0000_0000_0000)
        } else {
            (a, b)
        };
    ...
    match sign_ctrl {
        0 => z = (z & !0x8000_0000_0000_0000) | (a & 0x8000_0000_0000_0000), // keep the sign of a
        1 => {}                              // keep the comparison result's sign
        2 => z &= !0x8000_0000_0000_0000,                     // force positive
        _ => z |= 0x8000_0000_0000_0000,                      // force negative
    }
    z
}
