...
pub fn round_pack_to_f32(sign: bool, exp: i16, sig: u32, status: &mut SoftFloatStatus) -> float32 {
    ...
    if 0xFD <= (exp.wrapping_sub(1) as u16) {
        if exp < 0 {
            ...
            return pack_to_f32(sign, exp, sig & 0x007FFFFF);
        }
...
pub fn round_pack_to_f64(sign: bool, exp: i16, sig: u64, status: &mut SoftFloatStatus) -> float64 {
    ...
    if 0x7FD <= (exp.wrapping_sub(1) as u16) {
        if exp < 0 {
            ...
            return pack_to_f64(sign, exp, sig & 0x000FFFFFFFFFFFFF);
        }
