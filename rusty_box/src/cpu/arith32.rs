...
pub fn ADD_EdGd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
...
pub fn ADD_GdEd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
...
pub fn ADD_EdId_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let op2 = instr.id();
...
/// Decoder swaps: src() = meta_data[1] = nnn = SOURCE register
pub fn SUB_EdGd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let op2 = cpu.get_gpr32(instr.src() as usize);
...
pub fn SUB_GdEd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1 = cpu.get_gpr32(instr.dst() as usize);
    let op2 = cpu.read_virtual_dword(seg, eaddr)?;
...
pub fn SUB_EdId_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let op2 = instr.id();
...
/// Decoder swaps: src() = meta_data[1] = nnn = SOURCE register
pub fn CMP_EdGd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
...
pub fn CMP_GdEd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1 = cpu.get_gpr32(instr.dst() as usize);
    let op2 = cpu.read_virtual_dword(seg, eaddr)?;
...
pub fn CMP_EdId_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1 = cpu.read_virtual_dword(seg, eaddr)?;
    let op2 = instr.id();
...
/// Decoder swaps: src() = meta_data[1] = nnn = SOURCE register
pub fn ADC_EdGd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let op2 = cpu.get_gpr32(instr.src() as usize);
...
pub fn ADC_GdEd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1 = cpu.get_gpr32(instr.dst() as usize);
    let op2 = cpu.read_virtual_dword(seg, eaddr)?;
...
pub fn NEG_EdM<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let result = 0u32.wrapping_sub(op1);
...
pub fn NEG_Ed<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
...
pub fn SBB_EdGd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let (op1, laddr) = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let op2 = cpu.get_gpr32(instr.src() as usize);
...
pub fn SBB_EdGd<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
...
pub fn SBB_GdEd_M<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1 = cpu.get_gpr32(instr.dst() as usize);
    let op2 = cpu.read_virtual_dword(seg, eaddr)?;
...
pub fn SBB_GdEd<I: BxCpuIdTrait>(cpu: &mut BxCpuC<I>, instr: &Instruction) -> Result<(), crate::cpu::CpuError> {
