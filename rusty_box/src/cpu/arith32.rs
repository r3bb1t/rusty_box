...
pub fn CMPXCHG_EdGd_M<I: BxCpuIdTrait>(
    ...
    let eaddr = cpu.resolve_addr32(instr);
    let seg = BxSegregs::from(instr.seg());
    let op1_32 = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    let diff_32 = cpu.eax().wrapping_sub(op1_32);
    cpu.update_flags_sub32(cpu.eax(), op1_32, diff_32);
...
pub fn CMPXCHG8B<I: BxCpuIdTrait>(
    ...
    let lo = cpu.read_rmw_virtual_dword(seg, eaddr)?;
    // Save address_xlation state for first dword
    let xlat_saved = cpu.address_xlation;
    let hi = cpu.read_virtual_dword(seg, eaddr.wrapping_add(4))?;
    ...
    if op1_64 == op2_64 {
        ...
        cpu.write_virtual_dword(seg, eaddr.wrapping_add(4), (src_64 >> 32) as u32)?;
