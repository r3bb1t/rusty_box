...
impl<I: BxCpuIdTrait> BxCpuC<'_, I> {
    ...
    pub fn evex_vpexpandd(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        let mut result = if zmask {
            BxPackedZmmRegister { zmm64u: [0; 8] }
        } else {
    ...
    pub fn evex_vpmovdb_r(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        unsafe {
            ...
            for i in 0..nelements {
                if (mask >> i) & 1 != 0 {
                    result.zmmubyte[i] = src.zmm32u[i] as u8;
                } else if zmask {
                    result.zmmubyte[i] = 0;
                } else {
                    // merge: keep original destination byte
                    result.zmmubyte[i] = dst_orig.zmmubyte[i];
                }
            }
    ...
    pub fn evex_vpmovdw_r(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        unsafe {
            ...
            for i in 0..nelements {
                if (mask >> i) & 1 != 0 {
                    result.zmm16u[i] = src.zmm32u[i] as u16;
                } else if zmask {
                    result.zmm16u[i] = 0;
                } else {
                    result.zmm16u[i] = dst_orig.zmm16u[i];
                }
            }
    ...
    pub fn evex_vpmovqd_r(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        unsafe {
            ...
            for i in 0..nelements {
                if (mask >> i) & 1 != 0 {
                    result.zmm32u[i] = src.zmm64u[i] as u32;
                } else if zmask {
                    result.zmm32u[i] = 0;
                } else {
                    result.zmm32u[i] = dst_orig.zmm32u[i];
                }
            }
    ...
    pub fn evex_vplzcntd(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        unsafe {
            for i in 0..nelements {
                result.zmm32u[i] = src.zmm32u[i].leading_zeros();
            }
        }
    ...
    pub fn evex_vplzcntq(&mut self, instr: &Instruction) -> super::Result<()> {
        ...
        unsafe {
            for i in 0..nelements {
                result.zmm64u[i] = src.zmm64u[i].leading_zeros() as u64;
            }
        }
