...
impl<I: BxCpuIdTrait> BxCpuC<'_, I> {
    ...
    ///
    /// Bochs: BX_CPU_C::stack_return_to_v86() in vm8086.cc:43-124
    ///
    ...
    pub(super) fn stack_return_to_v86(
        ...
        // and CPL == 0 to get here (Bochs vm8086.cc:51-52)
        debug_assert_eq!(
        ...
        // Load SS:ESP from stack (vm8086.cc:89-90)
        ...
        // Load ES, DS, FS, GS from stack (vm8086.cc:93-96)
        ...
        // Write EFLAGS with valid mask (vm8086.cc:102)
        // This sets the VM bit, transitioning to V8086 mode
        self.write_eflags(flags32, EFlags::VALID_MASK.bits());

        // Load CS:EIP (vm8086.cc:105-106)
        self.sregs[BxSegregs::Cs as usize].selector.value = raw_cs_selector as u16;
        self.set_eip(new_eip & 0xFFFF); // EIP masked to 16-bit (vm8086.cc:106)

        // Load remaining segment selectors (vm8086.cc:108-113)
        ...
        self.set_esp(new_esp); // Full 32-bit ESP loaded (vm8086.cc:114)

        // Initialize V8086 segment caches (vm8086.cc:116)
    ...
    ///
    /// Bochs: BX_CPU_C::iret16_stack_return_from_v86() in vm8086.cc:132-173
    ///
    ...
    pub(super) fn iret16_stack_return_from_v86(&mut self) -> super::Result<()> {
        ...
        // IOPL < 3 without VME → trap to V86 monitor (vm8086.cc:133-136)
        ...
        // VME path: IOPL < 3 but CR4.VME enabled (vm8086.cc:146-168)
        if self.cr4.vme() && iopl < 3 {
            // Check VIP+IF or TF → #GP(0) (vm8086.cc:148-152)
            ...
            // Load CS:IP (vm8086.cc:155-156)
            self.load_seg_reg_real_mode(BxSegregs::Cs, cs_raw);
            self.set_eip(ip as u32);

            // IF, IOPL unchanged; EFLAGS.VIF = TMP_FLAGS.IF (vm8086.cc:159-162)
        ...
        // Non-VME path: IOPL == 3 (vm8086.cc:170-172)
        self.load_seg_reg_real_mode(BxSegregs::Cs, cs_raw);
        self.set_eip(ip as u32);
        // write_flags with change_IOPL=false, change_IF=true (vm8086.cc:172)
    ...
    ///
    /// Bochs: BX_CPU_C::v86_redirect_interrupt() in vm8086.cc:203-267
    ///
    ...
    pub(super) fn v86_redirect_interrupt(&mut self, vector: u8) -> super::Result<bool> {
        if self.cr4.vme() {
            ...
            // Read I/O permission bitmap base from TSS offset 102 (vm8086.cc:213)
            ...
            // Read the redirection bitmap byte (vm8086.cc:219)
            let vme_redirection_bitmap = self.system_read_byte(tr_base + offset as u64)?;

            // If bit is 0, redirect through virtual-mode IVT (vm8086.cc:220)
            if (vme_redirection_bitmap & (1 << (vector & 7))) == 0 {
                // Redirect interrupt through virtual-mode IVT (vm8086.cc:223-260)
                ...
                // IOPL < 3: adjust flags for VME (vm8086.cc:232-238)
                if self.eflags.iopl() < 3 {
                ...
                // Push flags, CS, IP onto V86 stack (vm8086.cc:241-244)
                self.push_16(temp_flags)?;
                self.push_16(old_cs)?;
                self.push_16(old_ip)?;

                // Load new CS:IP from IVT (vm8086.cc:246-247)
                self.load_seg_reg_real_mode(BxSegregs::Cs, temp_cs);
                self.set_eip(temp_ip as u32);

                // Clear TF, RF (vm8086.cc:249-250)
        ...
        // Interrupt is not redirected or VME is OFF (vm8086.cc:258-264)
        if self.eflags.iopl() < 3 {
            tracing::debug!(
    ...
    ///
    /// Bochs: BX_CPU_C::init_v8086_mode() in vm8086.cc:269-294
    ///
    ...
    pub(super) fn init_v8086_mode(&mut self) {
        for sreg in 0..6 {
            // Set cache fields (vm8086.cc:270-282)
        ...
        self.handle_cpu_mode_change(); // vm8086.cc:286

        // Update alignment check state for the CPL change
        self.handle_alignment_check(); // vm8086.cc:289

        // Invalidate stack cache (vm8086.cc:292)
