...
/// The wrapper is type-safe and will work with any `BxCpuC<T>` instance.
///
/// # Example
/// ```ignore
...
pub(super) struct BxOpcodeEntry<T: crate::cpu::instrumentation::Instrumentation> {
    ...
    pub(super) execute1: fn(&mut BxCpuC<'_, T>, &Instruction) -> Result<()>,

    /// Handler function for register form or secondary handler
    /// None if instruction doesn't have a register form
    #[allow(clippy::type_complexity)]
    pub(super) execute2: Option<fn(&mut BxCpuC<'_, T>, &Instruction) -> Result<()>>,
...
pub(super) fn get_opcode_entry<T: crate::cpu::instrumentation::Instrumentation>(
    opcode: crate::cpu::decoder::Opcode,
) -> Option<BxOpcodeEntry<T>> {
    ...
    match opcode {
        ...
        Opcode::Nop => Some(BxOpcodeEntry {
            execute1: |_cpu: &mut BxCpuC<'_, T>, _instr: &Instruction| -> Result<()> { Ok(()) },
            execute2: None,
            opflags: OpFlags::empty(),
        }),
