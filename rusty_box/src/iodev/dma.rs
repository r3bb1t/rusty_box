...
/// Index to find channel from page register number (Bochs dma.cc:229)
...
/// DMA channel mode fields (Bochs dma.h:78-84)
...
/// State for a single DMA channel (Bochs dma.h:78-91)
#[derive(Debug, Clone, Default)]
pub struct DmaChannel {
    /// Channel mode fields (Bochs dma.h:78-84)
    pub(crate) mode: DmaChannelMode,
    /// Base address register (Bochs dma.h:85)
    pub(crate) base_address: u16,
    /// Current address register (Bochs dma.h:86)
    pub(crate) current_address: u16,
    /// Base count register (Bochs dma.h:87)
    pub(crate) base_count: u16,
    /// Current count register (Bochs dma.h:88)
    pub(crate) current_count: u16,
    /// Page register (bits 16-23 of physical address, Bochs dma.h:89)
    pub(crate) page_reg: u8,
    /// Channel in use by a device (Bochs dma.h:90)
    pub(crate) used: bool,
}

/// 8237 DMA Controller state (one chip, Bochs dma.h:69-91)
#[derive(Debug, Clone)]
pub struct Dma8237 {
    ...
    /// Mask bits per channel (Bochs dma.h:73)
    pub(crate) mask: [bool; 4],
    /// Flip-flop for address/count byte access (Bochs dma.h:74)
    pub(crate) flip_flop: bool,
    /// Status register (Bochs dma.h:75)
    pub(crate) status_reg: u8,
    /// Command register (Bochs dma.h:76)
    pub(crate) command_reg: u8,
    /// Controller disabled (command register bit 2, Bochs dma.h:77)
    pub(crate) ctrl_disabled: bool,
    /// Channel state (Bochs dma.h:78-91)
...
impl Dma8237 {
    ...
    /// Reset controller (Bochs dma.cc:186-195 reset_controller)
    fn reset(&mut self) {
...
/// DMA 8-bit channel handler function pointer types (Bochs dma.h:100-101)
///
...
/// DMA 16-bit channel handler function pointer types (Bochs dma.h:102-103)
pub type DmaRead16Handler = fn(data_word: &[u16], maxlen: u16) -> u16;
pub type DmaWrite16Handler = fn(data_word: &mut [u16], maxlen: u16) -> u16;

/// Per-channel DMA handler registrations (Bochs dma.h:99-104)
...
/// Dual DMA Controller System (Bochs dma.h:33-105)
#[derive(Debug)]
pub struct BxDmaC {
    /// Controller state: s[0] = DMA1, s[1] = DMA2 (Bochs dma.h:92)
    pub(crate) s: [Dma8237; 2],
    /// Hold Acknowledge (Bochs dma.h:94)
    hlda: bool,
    /// Terminal Count (Bochs dma.h:95)
    tc: bool,
    /// Extra page registers (Bochs dma.h:97)
...
impl BxDmaC {
    ...
    /// Initialize the DMA controllers (Bochs dma.cc:123-177)
    pub fn init(&mut self) {
        ...
        // Init all channel state (Bochs dma.cc:156-169)
        for i in 0..2 {
            for c in 0..4 {
        ...
        // Channel 4 is cascade (Bochs dma.cc:171-172)
    ...
    /// Reset the DMA controllers (Bochs dma.cc:179-183)
    pub fn reset(&mut self) {
        self.reset_controller(0);
        self.reset_controller(1);
    }

    /// Reset a single controller (Bochs dma.cc:185-195)
    ...
    // -----------------------------------------------------------------------
    // DMA handler registration (Bochs dma.cc:70-116)
    // -----------------------------------------------------------------------

    /// Register an 8-bit DMA channel handler (Bochs dma.cc:70-88)
    ...
    /// Register a 16-bit DMA channel handler (Bochs dma.cc:90-109)
    ...
    /// Unregister a DMA channel (Bochs dma.cc:111-116)
    ...
    /// Get Terminal Count state (Bochs dma.cc:118-121)
    pub fn get_tc(&self) -> bool {
        self.tc
    }

    // -----------------------------------------------------------------------
    // DMA control logic (Bochs dma.cc:571-791)
    // -----------------------------------------------------------------------

    /// Set DRQ line for a channel (Bochs dma.cc:571-620)
    pub fn set_drq(&mut self, channel: usize, val: bool) {
        ...
        if !val {
            // Clear request bit in status reg (Bochs dma.cc:588-589)
            self.s[ma_sl].status_reg &= !(1 << (ch + 4));
            self.control_hrq(ma_sl);
            return;
        }

        // Set request bit in status reg (Bochs dma.cc:595)
        self.s[ma_sl].status_reg |= 1 << (ch + 4);

        // Validate mode type (Bochs dma.cc:597-603)
        ...
        // Boundary check (Bochs dma.cc:605-617)
    ...
    /// Check DRQ lines and assert/deassert HRQ to CPU (Bochs dma.cc:622-653)
    fn control_hrq(&mut self, ma_sl: usize) {
        // Do nothing if controller is disabled (Bochs dma.cc:627-628)
        ...
        // Find highest priority channel with active unmasked DRQ (Bochs dma.cc:640-652)
        for ch in 0..4 {
    ...
    /// Raise HLDA — perform the actual DMA transfer (Bochs dma.cc:655-791)
    ///
    ...
    pub fn raise_hlda(&mut self) {
        ...
        // Find highest priority channel on DMA2 first (Bochs dma.cc:665-671)
        ...
        // If channel 0 on DMA2 (cascade), look at DMA1 (Bochs dma.cc:672-681)
        if channel == 0 {
        ...
        // No channel found — wait till unmasked (Bochs dma.cc:682-685)
        if channel >= 4 {
            return;
        }

        // Compute physical address (Bochs dma.cc:687-688)
        ...
        // Compute maxlen and TC (Bochs dma.cc:690-699)
        let mut maxlen: u16;
        ...
        match transfer_type {
            ...
            0 => {
                // Verify transfer (Bochs dma.cc:741-754)
                if ma_sl == 0 {
                    if let Some(handler) = self.handlers[channel].dma_write8 {
            ...
            _ => {
                // transfer_type 3 is undefined (Bochs dma.cc:756)
                tracing::error!("DMA: hlda: transfer_type 3 is undefined");
                return;
            }
        }

        // Set DACK (Bochs dma.cc:759)
        self.s[ma_sl].dack[channel] = true;

        // Update address and count (Bochs dma.cc:762-766)
        if !self.s[ma_sl].chan[channel].mode.address_decrement {
        ...
        // Check for count expiration (0xFFFF means underflow, Bochs dma.cc:767-790)
        if self.s[ma_sl].chan[channel].current_count == 0xFFFF {
            ...
            if !self.s[ma_sl].chan[channel].mode.autoinit_enable {
                // Set mask bit if not in autoinit mode (Bochs dma.cc:773)
                self.s[ma_sl].mask[channel] = true;
            } else {
                // Autoinit: reload count and base address (Bochs dma.cc:776-780)
                self.s[ma_sl].chan[channel].current_address =
    ...
    // -----------------------------------------------------------------------
    // I/O port read handler (Bochs dma.cc:233-354)
    // -----------------------------------------------------------------------

    /// Read from DMA I/O port (Bochs dma.cc:233-354)
    pub fn read(&mut self, address: u16, _io_len: u8) -> u32 {
        let ma_sl: usize = if address >= 0xC0 { 1 } else { 0 };

        match address {
            // Current address registers (Bochs dma.cc:261-276)
            ...
            // Current count registers (Bochs dma.cc:278-293)
            ...
            // Status register (Bochs dma.cc:295-308)
            0x08 | 0xD0 => {
            ...
            // Temporary register (Bochs dma.cc:310-315)
            0x0D | 0xDA => {
                tracing::debug!(
            ...
            // Read all mask bits (undocumented, Bochs dma.cc:342-348)
            0x0F | 0xDE => {
    ...
    // -----------------------------------------------------------------------
    // I/O port write handler (Bochs dma.cc:360-569)
    // -----------------------------------------------------------------------

    /// Write to DMA I/O port (Bochs dma.cc:360-569)
    pub fn write(&mut self, address: u16, value: u32, io_len: u8) {
        // Handle word write to mode register (Bochs dma.cc:379-394)
        if io_len > 1 {
        ...
        match address {
            // Address registers (Bochs dma.cc:407-427)
            0x00 | 0x02 | 0x04 | 0x06 | 0xC0 | 0xC4 | 0xC8 | 0xCC => {
                let channel =
                    ((address >> (1 + ma_sl as u16)) & 0x03) as usize;
                if !self.s[ma_sl].flip_flop {
            ...
            // Count registers (Bochs dma.cc:429-449)
            0x01 | 0x03 | 0x05 | 0x07 | 0xC2 | 0xC6 | 0xCA | 0xCE => {
                let channel =
                    ((address >> (1 + ma_sl as u16)) & 0x03) as usize;
                if !self.s[ma_sl].flip_flop {
            ...
            // Command register (Bochs dma.cc:451-459)
            0x08 | 0xD0 => {
                if (value & 0xFB) != 0x00 {
                    tracing::debug!(
            ...
            // Request register (Bochs dma.cc:461-475)
            0x09 | 0xD2 => {
            ...
            // Single mask register (Bochs dma.cc:477-485)
            0x0A | 0xD4 => {
                let channel = (value & 0x03) as usize;
            ...
            // Mode register (Bochs dma.cc:487-495)
            0x0B | 0xD6 => {
                let channel = (value & 0x03) as usize;
            ...
            // Clear byte flip/flop (Bochs dma.cc:497-500)
            0x0C | 0xD8 => {
                tracing::debug!("DMA-{}: clear flip/flop", ma_sl + 1);
                self.s[ma_sl].flip_flop = false;
            }

            // Master clear (Bochs dma.cc:503-511)
            0x0D | 0xDA => {
                tracing::debug!("DMA-{}: master clear", ma_sl + 1);
                self.reset_controller(ma_sl);
            }

            // Clear mask register (Bochs dma.cc:513-521)
            0x0E | 0xDC => {
            ...
            // Write all mask bits (Bochs dma.cc:523-531)
            0x0F | 0xDE => {
            ...
            // DMA1 page registers (Bochs dma.cc:533-541)
            0x81 | 0x82 | 0x83 | 0x87 => {
                let channel = CHANNEL_INDEX[(address - 0x81) as usize] as usize;
            ...
            // DMA2 page registers (Bochs dma.cc:543-551)
            0x89 | 0x8A | 0x8B | 0x8F => {
                let channel = CHANNEL_INDEX[(address - 0x89) as usize] as usize;
            ...
            // Extra page registers (Bochs dma.cc:553-563)
            0x0080 | 0x0084 | 0x0085 | 0x0086 | 0x0088 | 0x008C | 0x008D | 0x008E => {
