...
/// Bochs scancodes.cc:33 — `translation8042[256]`.
...
pub struct BxKeyboardC {
    ...
    /// Set 2→1 translation escape flag. When `scancodes_translate` is true and a
...
impl BxKeyboardC {
    ...
    ///
    /// Matches Bochs gen_scancode() translation logic (keyboard.cc:677-696).
    pub fn send_scancode(&mut self, scancode: u8) {
        ...
        if self.kbd_controller.scancodes_translate {
            // Set 2 → Set 1 translation (Bochs keyboard.cc:677-689)
            if scancode == 0xF0 {
                // 0xF0 = Set 2 break prefix: set escaped flag, don't enqueue
                self.scancode_escaped = true;
            } else {
                let escaped = if self.scancode_escaped { 0x80u8 } else { 0x00u8 };
                self.scancode_escaped = false;
