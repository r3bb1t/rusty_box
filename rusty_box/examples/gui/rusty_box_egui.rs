...
fn run_emulator(
    ...
    // Profile-specific media, plus the direct-boot payload the machine needs
    // once it exists.
    ...
    match profile {
        BootProfile::Dlx { disk_path } => {
            disk_path_str = disk_path.to_string_lossy().to_string();
            builder = builder
        ...
        BootProfile::Alpine { iso_path, .. } => {
            iso_path_str = iso_path.to_string_lossy().to_string();
            builder = builder
                .boot_order(BootOrder::just(BootDevice::Cdrom))
                .cdrom_file(AtaSlot::SECONDARY_MASTER, &iso_path_str);
            println!("Alpine CD-ROM attached: {}", iso_path.display());
        }
        BootProfile::AlpineDirect { iso_path, .. } => {
            // Direct kernel boot — the ISO still rides along so Alpine's init
            // can mount the squashfs root filesystem.
            iso_path_str = iso_path.to_string_lossy().to_string();
            builder = builder.cdrom_file(AtaSlot::SECONDARY_MASTER, &iso_path_str);
            ...
            direct_boot = Some((kernel, initramfs, cmdline));
        }
    }
